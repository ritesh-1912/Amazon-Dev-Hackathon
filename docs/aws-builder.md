...
|-----------|-----------|---------------------------|
| **AI Layer Wrapper** | [`src/lib/bedrock.ts`](../src/lib/bedrock.ts) (re-exported at [`lib/bedrock.ts`](../lib/bedrock.ts)) | Initializes `BedrockRuntimeClient`, formats structured dependency states, defines calm academic ops system prompt, and executes `ConverseCommand`. |
...
| **Environment Configuration** | [`.env.example`](../.env.example) | Specifies `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `BEDROCK_MODEL_ID`. |
| **Unit & Resilience Tests** | [`tests/phase3.test.ts`](../tests/phase3.test.ts) | Tests Bedrock request formatting, client execution, and graceful fallback behavior. |
